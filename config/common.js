...
export default new class newCommon {
    ...
    async getPluginsName() {
        let files = fs.readdirSync("./plugins/example", { withFileTypes: true });
        let ret = [];
        for (let val of files) {
          let filepath = "../../example/" + val.name;
          let tmp = {
            name: val.name,
          };
          if (val.isFile()) {
     ...
     * @return {Boolean}
     */
    async getforwardMsg(e, message, dec = '', time = 0, isBot = true, isfk = true) {
        let forwardMsg = []
