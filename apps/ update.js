...
export class BtcUpdate extends plugin {
    ...
    async runUpdate(isForce) {
        const _path = './plugins/biscuit-plugin/' 
        ...
        this.oldCommitId = await this.getcommitId('biscuit-plugin')
        ...
        if (ret.error) {
            logger.mark(`${this.e.logFnc} 更新失败：Biscuit插件`)
            this.gitErr(ret.error, ret.stdout)
            return false
        }

        /** 获取插件提交的最新时间 */
        let time = await this.getTime('biscuit-plugin')

        if (/(Already up[ -]to[ -]date|已经是最新的)/.test(ret.stdout)) {
            await this.reply(`Biscuit插件已经是最新版本\n最后更新时间：${time}`)
        } else {
            await this.reply(`Biscuit插件\n最后更新时间：${time}`)
            this.isUp = true
            let log = await this.getLog('biscuit-plugin')
    ...
    async getLog(plugin = '') {
        ...
        let end = '更多详细信息，请前往gitee查看\nhttps://gitee.com/Yummy-cookie/biscuit-plugin'
